...
const loadShipImage = function (ship) {
    ...
    return loadImage(`${shipImages.shipImageUrl}/${id}/${shipImages.shipImageFilename}`)
        .then(image => ({id, image}))
        .catch(() => undefined);
};

const isValidShip = ship => ship != null && __.isInteger(parseInt(ship.mst_id));
//module.exports = {hello:"hello"};

const attachSingleFleetImages = function (replayData, availableWidth, availableHeight, ctx, canvas) {

    const involvedShips = __.filter(replayData.fleet1, isValidShip);

    return __.map(involvedShips, loadShipImage)
        .then(shipImages => {
            ...
            __.forEach(involvedShips, (ship, i) => {
                const imgDef = __.find(shipImages, {id: ship.mst_id});
                ...
                if (imgDef) {
                    ...
                    const ratioY = preferredDimensions.height / shipImages.shipImageDimensions.height;
...
const calculateSingleFleetPreferredDimensions = function (availableX, availableY, involvedShips) {
    ...
    let preferredDimensions = {width: shipImages.shipImageDimensions.width, height: shipImages.shipImageDimensions.height};
...
module.exports.generate =   function(w, h, replayData, poiData, owner) {
