...
/*
config = function(data,modMessage,threshold) {
 "t": 3,
 "threshold": 1,
 "codeUnitSize": 16,
 "args": function(i) { return i+1; },
 "messageDelimiter": function() {
...
module.exports.encode =  function(message, image, options) {
 options = options || {};
...
module.exports.decode =  function(image, options) {
 options = options || {};
 var config = this.config;
  ...
  for(var k = 0, done=false; !done; k+=1) {
   ...
   for(var i = 0; i < q.length; i+=1) {
    modMessage.push((((Math.pow(-1,q.length-(i+1))*util.sum(function(j) {
     return orderVariableCoefficients(i,j)*variableCoefficients[j];
...
/* module.exports.getHidingCapacity = function() {
  options = options || {};
  var myConfig = config({},'',1);
